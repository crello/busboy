...
function Multipart(boy, cfg) {
  ...
  if (typeof boundary !== 'string')
	...
	limits = limits || {};
	limits.fileSize = typeof limits.fileSize === 'number' ? limits.fileSize : Infinity;
	limits.fieldSize = typeof limits.fieldSize === 'number' ? limits.fieldSize : 1 * 1024 * 1024;
	limits.files = typeof limits.files === 'number' ? limits.files : Infinity;
	limits.fields = typeof limits.fields === 'number' ? limits.fields : Infinity;
	limits.parts = typeof limits.parts === 'number' ? limits.parts : Infinity;
    ...
    part.on('header', function(header) {
      ...
      if (contype === 'application/octet-stream' || filename !== undefined) {
        ...
        onData = function(data) {
          if ((nsize += data.length) > limits.fileSize) {
            var extralen = (limits.fileSize - (nsize - data.length));
            if (extralen > 0)
              file.push(data.slice(0, extralen));
            file.emit('limit', nsize);
            file.truncated = true;
        ...
        onData = function(data) {
          if ((nsize += data.length) > limits.fieldSize) {
            var extralen = (limits.fieldSize - (nsize - data.length));
